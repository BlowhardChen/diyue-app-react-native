...
const MechanicalTaskScreen: React.FC = () => {
  ...
  const viewMechanicalTaskDetail = (item: any) => {
    navigation.navigate("MechanicalTaskDetail", {id: item.id, navTitle: item.name});
  };

  // 渲染农事类型子项（犁地/旋耕等）
  const renderFarmingTypeItem = (item: any) => {
    return (
      <TouchableOpacity style={FarmingMapScreenStyles.farmingTypeItem} onPress={() => viewMechanicalTaskDetail(item)}>
